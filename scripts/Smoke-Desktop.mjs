...
await writeFile(queuePath, JSON.stringify({ queue: { version: 1, tasks: [], sessions: [{ taskId: 'legacy-deleted', title: 'Private legacy title', start: 1, end: 2 }], focusId: null, runningSince: null } }))
...
try {
  ...
  const trayAction = action => page.evaluate(action => globalThis.__TAURI_INTERNALS__.invoke('test_tray', { action }), action)
  const expectTaskbar = async visible => {
    await expect(async () => {
      const state = await trayAction('state')
      expect(state.trayExists).toBe(true)
      expect(state.visible).toBe(true)
      expect(await page.evaluate(() => globalThis.__lastSkipTaskbar)).toBe(!visible)
  ...
  const simulateFailures = () => {
    ...
    globalThis.fetch = async (input, options) => {
      ...
      if (url?.hostname === 'ipc.localhost') {
        ...
        if (command === globalThis.__failureCommand || command === `plugin:autostart|${globalThis.__startupFailure}` || (command === 'set_startup_enabled' && globalThis.__startupFailure === 'enable')) {
        ...
        if (command === 'plugin:window|set_skip_taskbar') {
          const response = await fetch(input, options)
          if (response.headers.get('Tauri-Response') === 'ok') globalThis.__lastSkipTaskbar = JSON.parse(options.body).value
          return response
        }
      }
  ...
  await expect(page.locator('.app-shell, .focus-overlay')).toBeVisible()
  await expect(async () => expect((await trayAction('state')).trayExists).toBe(true)).toPass({ timeout: 10000 })
  await page.addInitScript(simulateFailures)
  await page.reload()
  await expect(page.getByRole('dialog', { name: 'Welcome to Priority Queue' })).toBeVisible()
  await expectTaskbar(true)
  await page.evaluate(() => globalThis.__TAURI_INTERNALS__.invoke('initialize_tray'))
  expect(await windowCommand('is_always_on_top')).toBe(false)
  ...
  const startupChecked = await page.getByRole('checkbox', { name: 'Start with Windows' }).isChecked()
  await page.getByRole('button', { name: 'Privacy notice', exact: true }).click()
  await expect(page.getByRole('dialog', { name: 'Privacy notice' }).locator('.privacy-copy')).toContainText('Microsoft WebView2 diagnostics')
  await page.getByRole('button', { name: 'Back', exact: true }).click()
  expect(await page.getByRole('checkbox', { name: 'Start with Windows' }).isChecked()).toBe(startupChecked)
  ...
  expect(await windowCommand('is_resizable')).toBe(true)
  expect(await windowCommand('is_always_on_top')).toBe(false)
  await expect(page.locator('html')).toHaveCSS('background-color', 'rgb(17, 19, 21)')
  await expect(page.locator('.app-shell')).toHaveCSS('background-color', 'rgb(25, 28, 30)')
  ...
  await expect(page.getByRole('button', { name: 'Compact overlay (Ctrl+Shift+M)', exact: true })).toBeEnabled()
  await expectTaskbar(true)
  expect(await windowCommand('inner_size')).toEqual({ width: Math.round(620 * scale), height: Math.round(780 * scale) })
  expect(await windowCommand('is_always_on_top')).toBe(false)
  ...
  await page.getByLabel('Task', { exact: true }).fill('Unsaved tray draft')
  await trayAction('show-mini')
  await expect(page.getByRole('dialog', { name: 'New task', exact: true })).toBeVisible()
  await expect(page.getByLabel('Task', { exact: true })).toHaveValue('Unsaved tray draft')
  await page.getByRole('button', { name: 'Cancel', exact: true }).click()
  await page.getByRole('button', { name: 'Dismiss message', exact: true }).click()
  await trayAction('open-full')
  await expectTaskbar(true)
  ...
  await expect(page.getByRole('alert')).toHaveCount(0)
  expect(await windowCommand('is_always_on_top')).toBe(false)
  const readStoredQueue = () => page.evaluate(async () => (await globalThis.__TAURI_INTERNALS__.invoke('read_app_data', { kind: 'queue' })).queue)
  const beforeDeletion = await readStoredQueue()
  const deletedTask = beforeDeletion.tasks.find(task => task.title === 'Publish the infra plugin to playground')
  expect(deletedTask).toBeTruthy()
  expect(beforeDeletion.sessions.some(session => session.taskId === deletedTask.id)).toBe(true)
  await page.getByRole('button', { name: 'Completed 1', exact: true }).click()
  await page.getByRole('button', { name: 'Actions for Publish the infra plugin to playground', exact: true }).click()
  ...
  await expect(page.locator('.app-footer').getByRole('status')).toHaveText('Saved locally')
  const afterDeletion = await readStoredQueue()
  expect(afterDeletion.tasks.some(task => task.id === deletedTask.id)).toBe(false)
  expect(afterDeletion.sessions.some(session => session.taskId === deletedTask.id)).toBe(false)
  expect(JSON.stringify(afterDeletion)).not.toContain('Publish the infra plugin to playground')
  expect(afterDeletion.sessions.some(session => session.taskId === 'legacy-deleted')).toBe(true)
  await page.reload()
  await expect(expand).toBeEnabled()
  await expand.click()
  await expect(page.getByRole('button', { name: 'Compact overlay (Ctrl+Shift+M)', exact: true })).toBeEnabled()
  await page.getByRole('button', { name: 'Privacy', exact: true }).click()
  await page.getByRole('button', { name: 'Delete retained history', exact: true }).click()
  await page.getByRole('button', { name: 'Delete', exact: true }).click()
  await expect(page.getByRole('button', { name: 'Delete retained history', exact: true })).toBeDisabled()
  await page.getByRole('button', { name: 'Close', exact: true }).click()
  await expect(page.locator('.app-footer').getByRole('status')).toHaveText('Saved locally')
  await page.reload()
  await expect(expand).toBeEnabled()
  const afterCleanup = await readStoredQueue()
  expect(afterCleanup.tasks).toEqual(afterDeletion.tasks)
  expect(afterCleanup.sessions).toEqual(afterDeletion.sessions.filter(session => session.taskId !== 'legacy-deleted'))
  expect(JSON.stringify(afterCleanup)).not.toContain('Private legacy title')
  await expectTaskbar(false)
  await page.evaluate(() => { globalThis.__failureCommand = 'save_queue_data' })
  await trayAction('quit')
  await expect(page.getByRole('alert')).toContainText('Could not save. The window has been kept open.')
  expect(child.exitCode).toBeNull()
  await expectTaskbar(true)
  await page.evaluate(() => { globalThis.__failureCommand = '' })
  ...
  await writeFile(queuePath, encryptedQueue)
  await page.reload()
  await expect(expand).toBeEnabled()
  await expectTaskbar(false)
  const exited = once(child, 'exit', { signal: globalThis.AbortSignal.timeout(10000) })
  await trayAction('quit').catch(() => undefined)
  await exited
  log('Desktop smoke test passed: native tray actions, compact/full taskbar flags, overlay and keyboard expansion, editor protection, save-before-quit failure/retry, encrypted storage, privacy, overlay persistence and lock/unlock. Used an isolated debug profile; original startup entries are restored.')
} finally {
