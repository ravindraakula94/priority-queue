...
try {
  ...
  expect(await windowCommand('is_resizable')).toBe(false)
  ...
  await exited
  log('Desktop smoke test passed: compact startup, sizing, transparency, normal/maximized restoration, Windows lock/unlock in both views, paused auto-next, storage, pin/unpin and shutdown. Test tasks used a temporary store.')
} finally {
